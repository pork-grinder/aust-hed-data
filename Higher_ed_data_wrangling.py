...
def loadPerformance():
    ...
    '''a function to load, clean and join student success rate 
    and attrition rate data'''
...
def buildPostgrad(eftsl):
    ...
    df = eftsl.drop(['provider_code', 'ref_year'], axis=1).groupby([
            'Course_level','code_year']).sum().reset_index()
...
def buildInternational(eftsl):
    ...
    df = eftsl.drop(['provider_code', 'ref_year'], axis=1).groupby(
            ['citizenship','code_year']).sum().reset_index()
...
def buildBfoe(eftsl):
    ...
    df = eftsl.drop(['provider_code', 'ref_year'], axis=1).groupby([
            'primary_BFOE','code_year']).sum().reset_index()
...
def staffAdjust(df_imp):
    ...
    df_diff.loc[df_diff.TEQSA_type!='university', 'sessional_prop'] = df_diff.loc[df_diff.TEQSA_type!='university', 'sessional_prop'] + df_diff.loc[df_diff.TEQSA_type!='university', 'sessional_diff']
    df_diff.loc[df_diff.TEQSA_type!='university', 'ssr_all'] = df_diff.loc[df_diff.TEQSA_type!='university', 'ssr_all'] + df_diff.loc[df_diff.TEQSA_type!='university', 'ssr_all_diff']
